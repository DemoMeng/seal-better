...
public abstract class SealUtil {
    ...
    public static BufferedImage buildSeal(SealConfiguration conf) throws Exception {
        ...
        //1.画布
        BufferedImage bi = new BufferedImage(conf.getImageSize(), conf.getImageSize(), BufferedImage.TYPE_INT_RGB);
    ...
    public static BufferedImage buildPersonSeal(int imageSize, int lineSize, SealFont font, String addString)
        ...
        //1.画（设置背景图）
        BufferedImage bi = new BufferedImage(imageSize, imageSize / 2, BufferedImage.TYPE_INT_RGB);
        ...
        if (font.getFontText().length() == 2) {
            if (addString != null && addString.trim().length() > 0) {
                ...
                ng2d.drawRect(0, 0, imageSize, imageSize);//ng2d.drawRect(80,100,40,25);//画线框
                ng2d.dispose();
                bi = nbi;
            }
